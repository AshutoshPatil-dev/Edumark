...
// .wrangler/tmp/bundle-fTNdfK/checked-fetch.js
...
// .wrangler/tmp/pages-ZjNiCG/functionsWorker-0.35586639363023675.mjs
...
var onRequest = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const token = await new SignJWT({ userId: user.id, role: profile.role }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("24h").sign(new TextEncoder().encode(JWT_SECRET || "fallback_secret"));
...
var onRequest2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    const { email, password, fullName, role } = await context.request.json();
    ...
    const passwordBuffer = encoder3.encode(password);
    const baseKey = await crypto.subtle.importKey("raw", passwordBuffer, "PBKDF2", false, ["deriveBits"]);
    const derivedBits = await crypto.subtle.deriveBits(
      { name: "PBKDF2", salt, iterations: 1e5, hash: "SHA-256" },
      baseKey,
    ...
    await DB.prepare("INSERT INTO profiles (id, full_name, role) VALUES (?, ?, ?)").bind(userId, fullName, role).run();
    const token = await new SignJWT({ userId, role }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("24h").sign(new TextEncoder().encode(JWT_SECRET || "fallback_secret"));
...
// .wrangler/tmp/bundle-fTNdfK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fTNdfK/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.35586639363023675.js.map
